...
sazonality = function(selectionsResultSeries, statistic = "median"){
  ...
  MedianasMensais <- function(x) {
    mediaMovel <- x %>%
      ...
      dplyr::summarise(mediaMes = base::mean(stream_flow_m3_s,
                                             na.rm = TRUE
      ), medianaMes = stats::median(stream_flow_m3_s,
                                    na.rm = TRUE
      )) %>%
      base::suppressMessages() %>%
      dplyr::slice(rep(1:dplyr::n(), 2)) %>%
      dplyr::mutate(
        mediaMovel6 = zoo::rollapply(.data$medianaMes, 6,
                                     FUN = base::mean,
                                     partial = T, align = "left"
        ),
        medianaMovel6 = zoo::rollapply(.data$mediaMes,
                                       6,
                                       FUN = base::mean, partial = T, align = "left"
        ),
        meses = zoo::rollapply(.data$mes, 6, FUN = function(y) {
          paste(dplyr::first(y),
                dplyr::last(y),
                sep = "-"
          )
  ...
  if(statistic == "median"){
    ...
    AnoHS <- lapply(selectionsResultSeries, FUN = MedianasMensais) %>%
      ...
      dplyr::select(station_code, mes, medianaMes, medianaMovel6, meses) %>%
      dplyr::filter(medianaMovel6 == min(medianaMovel6)) %>%
      dplyr::select(1,2,5) %>%
      stats::setNames(c(
        "station_code", "FirstDryMonth",
        "DryMonths"
      ))
    ...
    AnoHU <- lapply(selectionsResultSeries, FUN = MedianasMensais) %>%
      ...
      dplyr::group_by(station_code) %>%
      dplyr::select(station_code, mes, medianaMes, medianaMovel6, meses) %>%
      dplyr::filter(medianaMovel6 == max(medianaMovel6)) %>%
      dplyr::select(
        1,
        2, 5
      ) %>%
      stats::setNames(c(
        "station_code", "FirstwetMonth",
        "WetMonths"
      ))
    ...
    AnoHS <- lapply(selectionsResultSeries, FUN = MedianasMensais) %>%
      dplyr::bind_rows() %>%
      dplyr::group_by(station_code) %>%
      dplyr::select(station_code, mes, mediaMes, mediaMovel6, meses) %>%
