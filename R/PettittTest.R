...
#' @importFrom rlang :=
#'
PettittTest <- function(dfSeriesFromFillorSerieStatisticsFunc, byMonth = FALSE, plotGraph = FALSE, dirSub = "./pettittTestGraph") { # se byMonth for igual a TRUE, faz o RunTest por mÊs da série mensal. Caso contrário na série mensal ou anual
  ...
  if (byMonth == FALSE) { # for time series
      ...
      if (plotGraph == TRUE) {
        ...
        dfSeriesFromFillorSerieStatisticsFunc2[[i]] %>%
          ...
          ggplot2::labs(
            x = period,
            y = ylab,
            title = paste("pettitt test (p.value = ", round(testPT$p.value, 3), ")", sep = ""),
    ...
    for (j in 1:length(dfSeriesFromFillorSerieStatisticsFunc2)) {
      ...
      for (i in 1:12) {
        ...
        #pettitt test for monthly i data of station j
        ...
        if (plotGraph == TRUE) {
          ...
          dfSeriesFromFillorSerieStatisticsFunc2[[j]] %>%
            ...
            ggplot2::labs(
              x = period,
              y = ylab,
              title = paste("pettitt test (p.value = ", round(testPT_m$p.value, 3), ")", sep = ""),
